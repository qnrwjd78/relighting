...
class UNet3DConditionModel(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        # Learned repeated-injection path for relight experiments. Each branch
        # starts from the same original light embedding and ends with a zero-
        # initialized residual projection. This keeps the new path as an exact
        # no-op at initialization time while giving every UNet scale its own
        # semantically aligned adapter to learn during continued finetuning.
        self.pose_cond_multiscale_projs = nn.ModuleList(
            [
                nn.Sequential(
                    InflatedConv3d(
    ...
    @staticmethod
    def _match_pose_condition_channels(
        pose_cond_fea: torch.Tensor, out_channels: int
    ) -> torch.Tensor:
        in_channels = pose_cond_fea.shape[1]
        if in_channels == out_channels:
            return pose_cond_fea

        if in_channels > out_channels:
            if in_channels % out_channels == 0:
                group_size = in_channels // out_channels
                new_shape = (
                    pose_cond_fea.shape[0],
                    out_channels,
                    group_size,
                    *pose_cond_fea.shape[2:],
                )
                return pose_cond_fea.reshape(new_shape).mean(dim=2)
            return pose_cond_fea[:, :out_channels]

        repeat_count = (out_channels + in_channels - 1) // in_channels
        expanded = pose_cond_fea.repeat(1, repeat_count, 1, 1, 1)[:, :out_channels]
        return expanded / (repeat_count ** 0.5)

    def _project_pose_condition(
        self,
        pose_cond_fea: torch.Tensor,
        sample: torch.Tensor,
        scale: float = 1.0,
    ) -> torch.Tensor:
        projected = pose_cond_fea
        ...
        projected = self._match_pose_condition_channels(projected, sample.shape[1])
        return projected.to(dtype=sample.dtype) * float(scale)

    def _build_multiscale_pose_condition(
        self,
        pose_cond_fea: torch.Tensor,
        ...
        # Build the repeated-injection feature for one specific UNet scale.
        # Each scale has its own adapter branch, but every branch starts from the
        # same original light embedding and is resized only for the current block.
        resized = pose_cond_fea
        if resized.shape[2:] != target_sample.shape[2:]:
            resized = F.interpolate(
                resized,
                size=target_sample.shape[2:],
                mode="trilinear",
                align_corners=False,
            )
        projected = self.pose_cond_multiscale_projs[projection_index](resized)
        return projected.to(dtype=target_sample.dtype) * float(scale)
    ...
    def forward(
        ...
        skip_mm: bool = False,
        use_multiscale_pose_cond: bool = False,
        pose_cond_scale: float = 0.25,
        ...
        # Original path: inject the pose/light feature once right after `conv_in`.
        if pose_cond_fea is not None:
            sample = sample + pose_cond_fea

        # Experimental relight path: keep the original one-shot injection above,
        # then optionally re-inject resized copies of the same condition at deeper
        # UNet resolutions so spatial lighting cues are seen more than once.
        # down
        ...
        for downsample_block in self.down_blocks:
            ...
            # Original scratch code kept for reference:
            # if pose_cond_fea is not None:
            #     sample = sample + pose_cond_fea[block_count]
            #     block_count += 1
            # Previous parameter-free experiment kept for reference:
            # if pose_cond_fea is not None and use_multiscale_pose_cond:
            #     sample = sample + self._project_pose_condition(
            #         pose_cond_fea,
            #         sample,
            #         scale=pose_cond_scale,
            #     )
            # Previous batched parallel-adapter experiment kept for reference:
            # target_multiscale_samples.append(sample)
            if pose_cond_fea is not None and use_multiscale_pose_cond:
                sample = sample + self._build_multiscale_pose_condition(
                    pose_cond_fea,
                    target_sample=sample,
                    projection_index=block_count,
                    scale=pose_cond_scale,
                )
                block_count += 1
        ...
        # Original implementation had no additional pose/light injection here.
        # Previous parameter-free experiment kept for reference:
        # if pose_cond_fea is not None and use_multiscale_pose_cond:
        #     sample = sample + self._project_pose_condition(
        #         pose_cond_fea,
        ...
        # target_multiscale_samples.append(sample)
        # if pose_cond_fea is not None and use_multiscale_pose_cond:
        #     multiscale_pose_cond = self._build_multiscale_pose_conditions(
        #         pose_cond_fea,
        #         target_samples=target_multiscale_samples,
        #         scale=pose_cond_scale,
        #     )
        #     for projected_pose_cond in multiscale_pose_cond[:-1]:
        #         target_multiscale_samples[block_count] = target_multiscale_samples[block_count] + projected_pose_cond
        #         block_count += 1
        #     sample = sample + multiscale_pose_cond[-1].to(dtype=sample.dtype)
        if pose_cond_fea is not None and use_multiscale_pose_cond:
            sample = sample + self._build_multiscale_pose_condition(
                pose_cond_fea,
                target_sample=sample,
