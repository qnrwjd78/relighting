...
class UNet3DConditionModel(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        # Multiscale light injection adapters (from unet_3d.py relight path).
        self.pose_cond_multiscale_projs = nn.ModuleList(
            [
                nn.Sequential(
                    InflatedConv3d(block_out_channels[0], block_out_channels[0], kernel_size=3, padding=1),
                    nn.SiLU(),
                    zero_module(InflatedConv3d(block_out_channels[0], block_out_channels[0], kernel_size=3, padding=1)),
                ),
                nn.Sequential(
                    InflatedConv3d(block_out_channels[0], block_out_channels[1], kernel_size=3, padding=1),
                    nn.SiLU(),
                    zero_module(InflatedConv3d(block_out_channels[1], block_out_channels[1], kernel_size=3, padding=1)),
                ),
                nn.Sequential(
                    InflatedConv3d(block_out_channels[0], block_out_channels[2], kernel_size=3, padding=1),
                    nn.SiLU(),
                    zero_module(InflatedConv3d(block_out_channels[2], block_out_channels[2], kernel_size=3, padding=1)),
                ),
                nn.Sequential(
                    InflatedConv3d(block_out_channels[0], block_out_channels[3], kernel_size=3, padding=1),
                    nn.SiLU(),
                    zero_module(InflatedConv3d(block_out_channels[3], block_out_channels[3], kernel_size=3, padding=1)),
                ),
                nn.Sequential(
                    InflatedConv3d(block_out_channels[0], block_out_channels[3], kernel_size=3, padding=1),
                    nn.SiLU(),
    ...
    def forward(
        ...
        # down (no reference attention — production hook path uses midup only)
